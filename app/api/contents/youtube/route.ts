...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      console.log(`[YouTubeAPI] Fetching transcript for video: ${video_id}`);
      transcript = await fetchTranscripts(video_id);
    } catch (error: unknown) {
      const message = error instanceof Error ? error.message : String(error);
      console.error(`[YouTubeAPI] Transcript fetch failed: ${message}`);
      return NextResponse.json(
    ...
    if (!transcript || transcript.length === 0) {
      console.warn(`[YouTubeAPI] Transcript is empty for video: ${video_id}`);
      return NextResponse.json(
        {
          message: "Error while processing youtube video: No transcript found",
        },
        { status: 400 }
      );
    }
    ...
    // metadata:
    console.log(`[YouTubeAPI] Fetching metadata for video: ${video_id}`);
    const metadataResponse = await axios.get(
      `https://www.googleapis.com/youtube/v3/videos?part=snippet,contentDetails&id=${video_id}&key=${process.env.YOUTUBE_APIKEY}`
    );
    const metadata = metadataResponse.data as {
      items: { snippet: VideoMetadata }[];
    };
    ...
    return NextResponse.json(
      {
        message: `Successfully added youtube content${
          existingYoutubeContent ? " (reused existing vectors)" : ""
        }`,
        data: {
          content_id: contentId,
          youtube_id: video_id,
    ...
    console.error(
      "Error while adding youtube content: ",
